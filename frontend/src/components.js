...
const ThreatMap = () => {
  ...
  // Generate realistic cyber threats with connections
  useEffect(() => {
    if (!mapImage) return;

    const threatCategories = [
      { name: 'DDoS', color: '#FF4D4D', icon: '⚡' },
      { name: 'Malware', color: '#FFA500', icon: '🦠' },
      { name: 'Phishing', color: '#FFD700', icon: '🎣' },
      { name: 'Ransomware', color: '#FF1493', icon: '🔒' },
      { name: 'Exploit', color: '#00BFFF', icon: '💥' },
      { name: 'Zero-Day', color: '#9370DB', icon: '0️⃣' },
      { name: 'APT', color: '#32CD32', icon: '🎯' },
      { name: 'Insider', color: '#FF69B4', icon: '👤' }
    ];

    // Major cyber attack hotspots with realistic distribution
    const hotspots = [
      // North America
      { name: 'New York', lat: 40.7128, lng: -74.0060, weight: 8 },
      { name: 'Silicon Valley', lat: 37.3875, lng: -122.0575, weight: 10 },
      { name: 'Washington DC', lat: 38.9072, lng: -77.0369, weight: 7 },
      // Europe
      { name: 'London', lat: 51.5074, lng: -0.1278, weight: 9 },
      { name: 'Frankfurt', lat: 50.1109, lng: 8.6821, weight: 7 },
      { name: 'Amsterdam', lat: 52.3676, lng: 4.9041, weight: 6 },
      // Asia
      { name: 'Tokyo', lat: 35.6762, lng: 139.6503, weight: 8 },
      { name: 'Singapore', lat: 1.3521, lng: 103.8198, weight: 7 },
      { name: 'Mumbai', lat: 19.0760, lng: 72.8777, weight: 6 },
      { name: 'Hong Kong', lat: 22.3193, lng: 114.1694, weight: 7 },
      // Other regions
      { name: 'Sydney', lat: -33.8688, lng: 151.2093, weight: 5 },
      { name: 'Sao Paulo', lat: -23.5505, lng: -46.6333, weight: 5 },
      { name: 'Dubai', lat: 25.2048, lng: 55.2708, weight: 6 },
      { name: 'Johannesburg', lat: -26.2041, lng: 28.0473, weight: 4 }
    ];

    const newThreats = [];

    // Generate threats based on hotspot weights
    hotspots.forEach(hotspot => {
      const threatCount = Math.ceil(hotspot.weight * (0.5 + Math.random() * 0.5));

      for (let i = 0; i < threatCount; i++) {
        const threatType = threatCategories[Math.floor(Math.random() * threatCategories.length)];

        // Add some randomness to the position
        const lat = hotspot.lat + (Math.random() * 4 - 2);
        const lng = hotspot.lng + (Math.random() * 8 - 4);

        newThreats.push({
          id: `${hotspot.name.replace(/\s+/g, '-').toLowerCase()}-${i}`,
          ...
          intensity: 0.3 + (Math.random() * 0.7 * (hotspot.weight / 10)),
          pulse: Math.random() * Math.PI * 2,
          connections: [],
          lastPing: 0,
          pingInterval: 1000 + Math.random() * 3000, // 1-4 seconds between pings
          attackTargets: []
        });
      }
    });

    // Create connections between threats
    newThreats.forEach((threat, i) => {
      // Each threat connects to 1-3 other threats
      const maxConnections = Math.min(3, Math.max(1, Math.floor(Math.random() * 3)));
      const connectedIndices = new Set();

      // Try to connect to threats of the same type first
      const sameTypeThreats = newThreats
        .map((t, idx) => ({ ...t, idx }))
        .filter((t, idx) =>
          idx !== i &&
          t.type === threat.type &&
          !connectedIndices.has(idx)
        );

      // Sort by distance
      sameTypeThreats.sort((a, b) => {
        const distA = Math.hypot(a.lat - threat.lat, a.lng - threat.lng);
        const distB = Math.hypot(b.lat - threat.lat, b.lng - threat.lng);
        return distA - distB;
      });

      // Add 0-2 same-type connections
      const sameTypeConnections = Math.min(
        Math.floor(Math.random() * 3),
        sameTypeThreats.length
      );

      for (let j = 0; j < sameTypeConnections && connectedIndices.size < maxConnections; j++) {
        connectedIndices.add(sameTypeThreats[j].idx);
      }
      ...
      while (connectedIndices.size < maxConnections) {
        ...
        if (randomIndex !== i && !connectedIndices.has(randomIndex)) {
          connectedIndices.add(randomIndex);
        }
      }

      // Create attack targets (subset of connections)
      const attackTargets = [];
      connectedIndices.forEach(idx => {
        if (Math.random() > 0.7) { // 30% chance to be an attack target
          attackTargets.push({
            targetIdx: idx,
            lastAttack: 0,
            attackInterval: 1000 + Math.random() * 5000, // 1-6 seconds between attacks
            active: false
          });
        }
      });

      threat.connections = Array.from(connectedIndices);
      threat.attackTargets = attackTargets;
    });
  ...
  // Project lat/lng to canvas coordinates with Mercator projection
  ...
  // Draw the threat map with real-world map and animated threats
  const drawThreatMap = (currentTime) => {
    ...
    // Draw world map with a dark overlay
    if (mapImage) {
      ...
      // Add color overlay for better visibility
      const gradient = ctx.createLinearGradient(0, 0, 0, height);
      gradient.addColorStop(0, 'rgba(10, 14, 23, 0.9)');
      gradient.addColorStop(1, 'rgba(10, 14, 23, 0.7)');
    ...
    threats.forEach((threat, i) => {
      const pulse = Math.sin(currentTime + threat.pulse) * 0.5 + 0.5;
      const pos = project(threat.lat, threat.lng, width, height);
      const baseRadius = 3 + threat.intensity * 4;
      const pulseRadius = baseRadius * (1 + pulse * 0.3);

      // Draw connection lines
      threat.connections.forEach(connIdx => {
        if (connIdx < threats.length) {
          const target = threats[connIdx];
          const targetPos = project(target.lat, target.lng, width, height);

          // Only draw lines to visible threats
          if (Math.abs(pos.x - targetPos.x) < width * 0.7 &&
            Math.abs(pos.y - targetPos.y) < height * 0.7) {

            // Check if this is an active attack
            const isAttack = threat.attackTargets.some(t => t.targetIdx === connIdx && t.active);

            if (isAttack) {
              // Draw animated attack line
              const progress = (Date.now() % 1000) / 1000; // 1 second animation
              const midX = pos.x + (targetPos.x - pos.x) * progress;
              const midY = pos.y + (targetPos.y - pos.y) * progress;

              const gradient = ctx.createLinearGradient(pos.x, pos.y, midX, midY);
              gradient.addColorStop(0, threat.color);
              gradient.addColorStop(1, target.color || '#FFFFFF');
              ...
              // Draw regular connection line
              const gradient = ctx.createLinearGradient(pos.x, pos.y, targetPos.x, targetPos.y);
              gradient.addColorStop(0, `${threat.color}40`);
              gradient.addColorStop(1, `${target.color || '#FFFFFF'}40`);
      ...
      if (threat.intensity > 0.5) {
        // Draw pulse effect for high-intensity threats
        const pulse = Math.sin(currentTime * 2) * 0.5 + 0.5;
        ctx.fillStyle = `${threat.color}${Math.floor(30 + pulse * 70).toString(16).padStart(2, '0')}`;
        ctx.beginPath();
        ctx.arc(pos.x, pos.y, pulseRadius * 1.5, 0, Math.PI * 2);
        ctx.fill();
      }

      // Draw threat icon
      if (threat.icon) {
        ctx.font = `${baseRadius * 1.5}px Arial`;
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillStyle = '#FFFFFF';
        ctx.fillText(threat.icon, pos.x, pos.y);
      }

      // Draw hover effect
      if (hoveredThreat === i) {
        // Draw circle around hovered threat
        ctx.strokeStyle = '#FFFFFF';
        ctx.lineWidth = 1.5;
        ctx.beginPath();
        ctx.arc(pos.x, pos.y, pulseRadius * 1.3, 0, Math.PI * 2);
        ctx.stroke();

        // Draw tooltip
        const tooltipText = `${threat.name}\n${threat.city}\n${threat.type}`;
        const lines = tooltipText.split('\n');
        const tooltipWidth = 120;
        const tooltipHeight = 20 + (lines.length * 16);
        const tooltipX = Math.min(pos.x, width - tooltipWidth - 10);
        const tooltipY = Math.max(10, pos.y - tooltipHeight - 10);

        // Tooltip background
        ctx.fillStyle = 'rgba(15, 23, 42, 0.9)';
        ctx.strokeStyle = threat.color;
        ctx.lineWidth = 1;
        roundRect(ctx, tooltipX, tooltipY, tooltipWidth, tooltipHeight, 5, true, true);
        ...
        // Draw connecting line to threat
        ctx.strokeStyle = `${threat.color}80`;
        ctx.lineWidth = 1;
        ctx.setLineDash([2, 2]);
        ctx.beginPath();
        ctx.moveTo(pos.x, pos.y - pulseRadius);
  ...
  // Animation loop with attack simulation
  useEffect(() => {
    ...
    // Function to simulate attacks between nodes
    const simulateAttacks = (time) => {
      setThreats(currentThreats => {
        return currentThreats.map(threat => {
          // Update attack targets
          const updatedAttackTargets = threat.attackTargets.map(target => {
            if (time - target.lastAttack > target.attackInterval) {
              // Trigger new attack
              return {
                ...target,
                lastAttack: time,
            ...
            } else if (time - target.lastAttack > 500) {
              // Deactivate attack after 500ms
              return {
                ...target,
                active: false
              };
          ...
          return {
            ...threat,
            attackTargets: updatedAttackTargets,
            pulse: (threat.pulse + 0.02) % (Math.PI * 2)
          };
        });
      });
    };
    ...
    const render = (currentTime) => {
      ...
      // Simulate attacks
      simulateAttacks(currentTime);

      // Draw the threat map
      drawThreatMap(currentTime);
    ...
    <div className="relative w-full h-full bg-gray-950 rounded-xl overflow-hidden">
      <canvas
        ...
        onMouseMove={(e) => {
          ...
          let minDist = 30; // Minimum distance to consider (in pixels)
          
          threats.forEach((threat, i) => {
            const pos = project(threat.lat, threat.lng, canvasRef.current.width, canvasRef.current.height);
      ...
      {/* Legend */}
      <div className="absolute bottom-4 right-4 bg-gray-900/80 backdrop-blur-sm p-3 rounded-lg border border-gray-700 text-xs">
        <div className="font-bold text-white mb-2">Threat Types</div>
        <div className="space-y-1">
          {[
            { color: '#FF4D4D', label: 'DDoS' },
            { color: '#FFA500', label: 'Malware' },
            { color: '#FFD700', label: 'Phishing' },
            { color: '#FF1493', label: 'Ransomware' },
            { color: '#00BFFF', label: 'Exploit' },
            { color: '#9370DB', label: 'Zero-Day' },
            { color: '#32CD32', label: 'APT' },
            { color: '#FF69B4', label: 'Insider' }
          ].map((item) => (
      ...
      {/* Status bar */}
      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-r from-blue-900/50 to-purple-900/50 p-2 text-xs text-gray-300 flex justify-between items-center">
        <div>
          {hoveredThreat !== null ? (
            <span>Selected: {threats[hoveredThreat]?.name} • {threats[hoveredThreat]?.type} • {Math.round(threats[hoveredThreat]?.intensity * 100)}% intensity</span>
          ) : (
            <span>Hover over a threat for details</span>
          )}
        </div>
        <div className="flex items-center">
          <div className="w-2 h-2 rounded-full bg-green-500 mr-1"></div>
          <span>Active Threats: {threats.filter(t => t.intensity > 0.5).length}</span>
        </div>
      </div>
    </div>
  );
